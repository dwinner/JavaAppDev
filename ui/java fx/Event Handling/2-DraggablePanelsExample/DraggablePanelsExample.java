package pkg2.draggablepanelsexample;
...
/**
 * Фильтры событый (распространение вниз).
 *
 * @author oracle_pr1
 */
    ...
    private Node makeDraggable(final Node node)
    {
        ...
        wrapGroup.addEventFilter(MouseEvent.ANY, new EventHandler<MouseEvent>()
        {
            ...
            public void handle(final MouseEvent mouseEvent)
            {
                ...
                {   // Отключить события мыши для остальных дочерних узлов
                    mouseEvent.consume();
                }
            }
        });
        ...
        wrapGroup.addEventFilter(MouseEvent.MOUSE_PRESSED, new EventHandler<MouseEvent>()
        {
            ...
            {
                if (dragModeActiveProperty.get())
                {   // Запомнить координаты мыши под курсором и позицию узла                    
        ...
        wrapGroup.addEventFilter(MouseEvent.MOUSE_DRAGGED, new EventHandler<MouseEvent>()
        {
            ...
            public void handle(final MouseEvent mouseEvent)
            {
                if (dragModeActiveProperty.get())
                {
                    // Перетащить узел из его исходной позиции на
                    // вычисленное значение сдвига под курсором.                       
