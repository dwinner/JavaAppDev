...
public class WebHighlighter extends LayeredHighlighter
{
    ...
    /**
     * Called when the UI is being installed into the interface of a JTextComponent.  Installs the
     * editor, and removes any existing highlights.
     *
    ...
    /**
     * Makes a copy of the highlights.  Does not actually clone each highlight, but only makes
     * references to them.
     *
     * @return the copy
     ...
     * <code>paintLayeredHighlight</code> which will result in a rectangle being drawn before the
     * text is drawn (if the offsets are in a highlighted region that is). For this to work the
     * painter supplied must be an instance of LayeredHighlightPainter.
     */
    ...
    /**
     * LayeredHighlightPainter is used when a drawsLayeredHighlights is true. It maintains a
     * rectangle of the region to paint.
     */
    class LayeredHighlightInfo extends HighlightInfo
    {
                                      ...
                                      JTextComponent editor, View view )
        {
            ...
            try
            {
                ...
                Area newClip = new Area ( new Rectangle ( insets.left - 2, insets.top - 2,
                        editor.getWidth () - insets.left - insets.right + 4,
                        editor.getHeight () - insets.top - insets.bottom + 4 ) );
                newClip.intersect ( new Area ( editor.getVisibleRect () ) );
                g2d.setClip ( newClip );

                // Корректная область выделения
                Rectangle b = ( Rectangle ) view
                        .modelToView ( p0, Position.Bias.Forward, p1, Position.Bias.Backward,
                                viewBounds );
                b.x -= 1;
                b.y -= 1;
                b.height += 1;
    ...
    /**
     * This class invokes <code>mapper.damageRange</code> in EventDispatchThread. The only one
     * instance per Highlighter is cretaed. When a number of ranges should be damaged it collects
     * them into queue and damages them in consecutive order in <code>run</code> call.
     */
    class SafeDamager implements Runnable
    {
        ...
        /**
         * Adds the range to be damaged into the range queue. If the range queue is empty (the first
         * call or run() was already invoked) then adds this class instance into EventDispatch
         * queue.
         * <p/>
         * The method also tracks if the current document changed or component is null. In this case
         * it removes all ranges added before from range queue.
         */
