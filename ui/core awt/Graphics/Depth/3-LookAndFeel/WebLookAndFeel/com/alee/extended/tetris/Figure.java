...
public class Figure
{
    ...
    {
        Point start = new Point ( point.x * Tetris.BLOCK_SIDE + point.x,
                point.y * Tetris.BLOCK_SIDE + point.y );
            ...
            for ( int col = 0; col < placement[ row ].length; col++ )
            {
                if ( placement[ row ][ col ] >= 0 )
                {
                    blocks[ placement[ row ][ col ] ].paintBlock ( g2d,
                            new Rectangle ( start.x + col * Tetris.BLOCK_SIDE + col,
                                    start.y + row * Tetris.BLOCK_SIDE + row, Tetris.BLOCK_SIDE,
                                    Tetris.BLOCK_SIDE ), rotation );
                }
            }
        }
    }
            ...
            for ( int col = 0; col < cols; col++ )
            {
                if ( previewData.get ( row ).get ( col ) >= 0 )
                {
                    blocks[ previewData.get ( row ).get ( col ) ].paintBlock ( g2d,
                            new Rectangle ( startX + col * Tetris.BLOCK_SIDE + col,
                                    startY + row * Tetris.BLOCK_SIDE + row, Tetris.BLOCK_SIDE,
                                    Tetris.BLOCK_SIDE ), rotation );
                }
            }
        }
            ...
            for ( int col = 0; col < placement[ row ].length; col++ )
            {
                if ( placement[ row ][ col ] >= 0 )
                {
                    ...
                    blocks[ placement[ row ][ col ] ]
                            .setBlockPoint ( new Point ( point.x + col, point.y + row ) );
                    blocksMap.put ( new Point ( start.x + col * Tetris.BLOCK_SIDE + col,
                            start.y + row * Tetris.BLOCK_SIDE + row ),
                            blocks[ placement[ row ][ col ] ] );
                }
            }
        }
