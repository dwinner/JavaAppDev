...
public class Tetris extends JComponent
{
    ...
    public static final Dimension PREFERRED_SIZE =
            new Dimension ( BLOCKS_IN_WIDTH * BLOCK_SIDE + BLOCKS_IN_WIDTH + 1,
                    BLOCKS_IN_HEIGHT * BLOCK_SIDE + BLOCKS_IN_HEIGHT + 1 );
    ...
    // Поле отображающее следующую фигуру
    private JComponent nextFigureField;

    // Использовать ли внутренние хоткеи
    ...
    public Tetris ()
    {
        ...
        keyTimer = new Timer ( 60, new ActionListener ()
        {
        ...
        pusher = new Timer ( getTickTime (), null )
        {
            public void start ()
            {
                gameRunning = true;
        ...
        addKeyListener ( new KeyAdapter ()
        {
            ...
            {
                if ( useInternalHotkeys && e.getKeyCode () == KeyEvent.VK_P )
                {
                    if ( gameRunning )
                    {
                        pauseGame ();
                ...
                else if ( useInternalHotkeys && e.getKeyCode () == KeyEvent.VK_F2 )
                {
                    newGame ();
                }
                else if ( keyTimer.isRunning () )
                {
                    if ( e.getKeyCode () == KeyEvent.VK_DOWN && action == Action.down )
                    {
                        keyTimer.stop ();
                    }
                    if ( !keyTimer.isRunning () )
    ...
    {
        // Если падающая фигура еще не выведена на поле добавляем её
        boolean cantMoveFuther =
                currentFigure != null && !currentFigure.canMoveDown ( getTerrainShape () );
        if ( currentFigure == null || cantMoveFuther )
        {
            if ( cantMoveFuther )
            {
                // Разбираем фигуру на блоки
                terrain.putAll ( currentFigure.getBlocks () );

                // Находим количество блоков в каждом уровне
                ...
                for ( Point point : terrain.keySet () )
                {
                    ...
                    blocksPerLevel.put ( level, blocksPerLevel.containsKey ( level ) ?
                            blocksPerLevel.get ( level ) + 1 : 1 );
                }

                // Сортируем уровни по возрастанию
                Collections.sort ( levels );

                // Проверяем нет ли заполненных строк
                for ( int level : levels )
                {
                    if ( blocksPerLevel.get ( level ) == BLOCKS_IN_WIDTH )
                    {
                        // Уничтожаем блоки заполнившие строку и сдвигаем все предыдущие уровни
                        ...
                        for ( Point point : terrain.keySet () )
                        {
                            ...
                            if ( lvl == level )
                            {
                                // Добавляем блок на удаление
                                toRemove.add ( point );
                            }
                            else if ( lvl < level )
                            {
                                // Добавляем блок на сдвиг
                                toMove.add ( point );
                            }
                        }
                        ...
                        // Сдвигаем блоки находившиеся выше
                        ...
                        for ( Point point : toMove )
                        {
                            ...
                            if ( lvl < level )
                            {
                                ...
                                movedBlocks.put ( new Point ( point.x, point.y + BLOCK_SIDE + 1 ),
                                        block );
                            }
                        }
            ...
            // todo Нужна иная проверка - не выходит ли какая фигура за верхний край
            // Если только появившаяся фигура не может спуститься - игра закончена
        ...
        // Сдвигаем (по возможности) фигуру вниз
    ...
    private GeneralPath getTerrainShape ()
    {
        ...
        for ( Point point : terrain.keySet () )
        {
            border.append ( new Rectangle ( point, new Dimension ( BLOCK_SIDE, BLOCK_SIDE ) ),
                    false );
        }
        return border;
    }
    ...
    private GeneralPath getBorderShape ()
    {
        ...
        border.append ( new Rectangle ( -BLOCK_SIDE - 1, -BLOCK_SIDE * 3 - 3, BLOCK_SIDE,
                PREFERRED_SIZE.height + BLOCK_SIDE * 3 + 3 ), false );
        border.append ( new Rectangle ( -BLOCK_SIDE - 1, PREFERRED_SIZE.height + 1,
                PREFERRED_SIZE.width + 2 + BLOCK_SIDE * 2, BLOCK_SIDE ), false );
        border.append ( new Rectangle ( PREFERRED_SIZE.width + 1, -BLOCK_SIDE * 3 - 3, BLOCK_SIDE,
                PREFERRED_SIZE.height + BLOCK_SIDE * 3 + 3 ), false );
        return border;
    }
    ...
    public static Stroke borderStroke =
            new BasicStroke ( 0.5f, BasicStroke.CAP_BUTT, BasicStroke.JOIN_BEVEL, 1f,
                    new float[]{ 4, 4 }, 0f );
    ...
    protected void paintComponent ( Graphics g )
    {
        ...
        {
            Block block = terrain.get ( point );
            block.paintBlock ( g2d,
                    new Rectangle ( point, new Dimension ( BLOCK_SIDE, BLOCK_SIDE ) ),
                    block.getRotation () );
        }
        ...
        if ( gameOver || !gameRunning )
        {
            ...
            GlyphVector gv = g2d.getFont ()
                    .createGlyphVector ( frc, gameOver ? gameOverText : gamePausedText );
            ...
            RoundRectangle2D rr = new RoundRectangle2D.Double ( getWidth () / 2 - width / 2 - 10,
                    getHeight () / 2 - bigTextBounds.height / 2 - smallFont.getSize () / 2 - 10,
                    width + 20, bigTextBounds.height + smallFont.getSize () + 20, 10, 10 );
            Rectangle bounds = rr.getBounds ();

            g2d.setPaint ( new GradientPaint ( 0, bounds.y, bgTop, 0, bounds.y + bounds.height,
                    bgBottom ) );
            g2d.fill ( rr );
            ...
            g2d.drawString ( gameOver ? gameOverText : gamePausedText,
                    getWidth () / 2 - bigTextBounds.width / 2,
                    getHeight () / 2 + bigTextBounds.height / 2 - smallFont.getSize () / 2 - 2 );

            g2d.setFont ( smallFont );
            g2d.setPaint ( Color.GRAY );
            g2d.drawString ( gameOver ? gameOverTip : gamePausedTip,
                    getWidth () / 2 - smallTextWidth / 2,
                    getHeight () / 2 + bigTextBounds.height / 2 + smallFont.getSize () / 2 + 2 );
        }
    }
    ...
    {
        for ( TetrisListener tetrisListener : tetrisListeners )
        {
