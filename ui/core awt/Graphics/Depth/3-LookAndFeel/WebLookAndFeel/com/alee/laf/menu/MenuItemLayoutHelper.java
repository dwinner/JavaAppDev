...
public class MenuItemLayoutHelper
{
    ...
    public MenuItemLayoutHelper ( JMenuItem mi, Icon checkIcon, Icon arrowIcon, Rectangle viewRect,
                                  int gap, String accDelimiter, boolean isLeftToRight, Font font,
                                  Font accFont, boolean useCheckAndArrow, String propertyPrefix )
    {
        reset ( mi, checkIcon, arrowIcon, viewRect, gap, accDelimiter, isLeftToRight, font, accFont,
                useCheckAndArrow, propertyPrefix );
    }

    protected void reset ( JMenuItem mi, Icon checkIcon, Icon arrowIcon, Rectangle viewRect,
                           int gap, String accDelimiter, boolean isLeftToRight, Font font,
                           Font accFont, boolean useCheckAndArrow, String propertyPrefix )
    {
        this.mi = mi;
        ...
        this.isColumnLayout =
                isColumnLayout ( isLeftToRight, horizontalAlignment, horizontalTextPosition,
    ...
    protected void calcWidthsAndHeights ()
    {
        ...
        if ( isColumnLayout )
        {
            ...
            labelSize.height =
                    max ( checkSize.height, iconSize.height, textSize.height, accSize.height,
                            arrowSize.height );
        }
        else
        {
            ...
            SwingUtilities.layoutCompoundLabel ( mi, fm, text, icon, verticalAlignment,
    ...
    protected void calcMaxWidths ()
    {
        ...
        if ( isColumnLayout )
        {
            ...
            labelSize.maxWidth = calcMaxValue ( MAX_LABEL_WIDTH,
                    iconSize.maxWidth + textSize.maxWidth + curGap );
        }
        else
        {
    ...
    /**
     * Answers should we do column layout for a menu item or not. We do it when a user doesn't set
     * any alignmentst set any alignments and text positions manually, except the vertical
     * alignment.
     */
    public static boolean isColumnLayout ( boolean isLeftToRight, int horizontalAlignment,
    ...
    /**
     * Calculates maximal text offset. It is required for some L&Fs (ex: Vista L&F). The offset is
     * meaningful only for L2R column layout.
     *
     ...
     * their positions.
     * <p/>
     * If horizontalAlignment, verticalTextPosition and horizontalTextPosition are default (user
     * doesn't set any manually) the layouting algorithm is:t set any manually) the layouting
     * algorithm is: Elements are layouted in the five columns: check icon + icon + text +
     * accelerator text + arrow icon
     * <p/>
     * In the other case elements are layouted in the four columns: check icon + label + accelerator
     * text + arrow icon Label is union of icon and text.
     * <p/>
    ...
    /**
     * Aligns the accelertor text and the check and arrow icons vertically with the center of the
     * label rect.
     */
    private void alignAccCheckAndArrowVertically ( LayoutResult lr )
    {
        lr.accRect.y = ( int ) ( lr.labelRect.y + ( float ) lr.labelRect.height / 2 -
                ( float ) lr.accRect.height / 2 );
        fixVerticalAlignment ( lr, lr.accRect );
        if ( useCheckAndArrow )
        {
            lr.arrowRect.y = ( int ) ( lr.labelRect.y + ( float ) lr.labelRect.height / 2 -
                    ( float ) lr.arrowRect.height / 2 );
            lr.checkRect.y = ( int ) ( lr.labelRect.y + ( float ) lr.labelRect.height / 2 -
    ...
    /**
     * Fixes vertical alignment of all menu item elements if rect.y or (rect.y + rect.height) is out
     * of viewRect bounds
     */
    ...
    private void doLTRColumnLayout ( LayoutResult lr, ColumnAlignment alignment )
    {
        ...
        calcXPositionsRTL ( viewRect.x + viewRect.width, leadingGap, gap, lr.arrowRect,
                lr.accRect );
    ...
    private void doRTLColumnLayout ( LayoutResult lr, ColumnAlignment alignment )
    {
        ...
        // Set X coordinates
        calcXPositionsRTL ( viewRect.x + viewRect.width, leadingGap, gap, lr.checkRect, lr.iconRect,
                lr.textRect );
    ...
    private void doRTLComplexLayout ( LayoutResult lr, ColumnAlignment alignment )
    {
        ...
        // Set X coordinates
        calcXPositionsRTL ( viewRect.x + viewRect.width, leadingGap, gap, lr.checkRect,
                lr.labelRect );
    ...
    protected void layoutIconAndTextInLabelRect ( LayoutResult lr )
    {
        ...
        SwingUtilities
                .layoutCompoundLabel ( mi, fm, text, icon, verticalAlignment, horizontalAlignment,
                        verticalTextPosition, horizontalTextPosition, lr.labelRect, lr.iconRect,
                        lr.textRect, gap );
    }
    ...
    private void calcTextAndIconYPositions ( LayoutResult lr )
    {
        ...
        {
            lr.textRect.y = ( int ) ( viewRect.y + ( float ) lr.labelRect.height / 2 -
                    ( float ) lr.textRect.height / 2 );
            lr.iconRect.y = ( int ) ( viewRect.y + ( float ) lr.labelRect.height / 2 -
                    ( float ) lr.iconRect.height / 2 );
        }
        else if ( verticalAlignment == SwingUtilities.CENTER )
        {
            lr.textRect.y = ( int ) ( viewRect.y + ( float ) viewRect.height / 2 -
                    ( float ) lr.textRect.height / 2 );
            lr.iconRect.y = ( int ) ( viewRect.y + ( float ) viewRect.height / 2 -
                    ( float ) lr.iconRect.height / 2 );
        }
        else if ( verticalAlignment == SwingUtilities.BOTTOM )
        {
            lr.textRect.y =
                    ( int ) ( viewRect.y + viewRect.height - ( float ) lr.labelRect.height / 2 -
                            ( float ) lr.textRect.height / 2 );
            lr.iconRect.y =
                    ( int ) ( viewRect.y + viewRect.height - ( float ) lr.labelRect.height / 2 -
                            ( float ) lr.iconRect.height / 2 );
    ...
    private void calcLabelYPosition ( LayoutResult lr )
    {
        ...
        {
            lr.labelRect.y = ( int ) ( viewRect.y + ( float ) viewRect.height / 2 -
    ...
    public static class LayoutResult
    {
        ...
        public LayoutResult ( Rectangle iconRect, Rectangle textRect, Rectangle accRect,
                              Rectangle checkRect, Rectangle arrowRect, Rectangle labelRect )
        {
    ...
    public static class ColumnAlignment
    {
        ...
        public static final ColumnAlignment LEFT_ALIGNMENT =
                new ColumnAlignment ( SwingConstants.LEFT, SwingConstants.LEFT, SwingConstants.LEFT,
                        SwingConstants.LEFT, SwingConstants.LEFT );

        public static final ColumnAlignment RIGHT_ALIGNMENT =
                new ColumnAlignment ( SwingConstants.RIGHT, SwingConstants.RIGHT,
                        SwingConstants.RIGHT, SwingConstants.RIGHT, SwingConstants.RIGHT );

        public ColumnAlignment ( int checkAlignment, int iconAlignment, int textAlignment,
                                 int accAlignment, int arrowAlignment )
        {
