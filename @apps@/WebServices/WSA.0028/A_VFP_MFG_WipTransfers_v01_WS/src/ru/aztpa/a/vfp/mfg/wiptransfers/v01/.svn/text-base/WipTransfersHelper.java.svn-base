...
class WipTransfersHelper
{
   ...
   ActionResult synchronizeTransferDocs(TransferDocs transferDocs)
   {
      ...
      for (int i = 0; i < transferDocs.getTransferDoc().size(); i++)
      {
         ...
         Integer site = currentDoc.getSite();   // Участок цеха отправителя ДСЕ
         if (site == null)
         {
         ...
         for (int j = 0; j < docLines.size(); j++)
         {
            ...
            Integer itemsPerBillet = docLine.getItemsPerBillet();  // Кол-во деталей в заготовке
            ...
            try
            {
               ...
               {
                  // FIXME: Запись уже существует -- updateTransfers.add(transfer);
