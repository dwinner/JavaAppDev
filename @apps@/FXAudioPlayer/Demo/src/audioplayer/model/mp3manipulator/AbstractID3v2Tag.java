...
public abstract class AbstractID3v2Tag implements ID3v2
{
    ...
    public AbstractID3v2Tag(byte[] bytes) throws NoSuchTagException, UnsupportedTagException,
                                                 InvalidDataException
    {
        this(bytes, false);
    }

    public AbstractID3v2Tag(byte[] bytes, boolean obseleteFormat) throws NoSuchTagException,
    ...
    private void unpackTag(byte[] bytes) throws NoSuchTagException, UnsupportedTagException,
                                                InvalidDataException
    {
    ...
    private int unpackHeader(byte[] bytes) throws UnsupportedTagException, InvalidDataException
    {
        ...
        dataLength = BufferTools.unpackSynchsafeInteger(bytes[DATA_LENGTH_OFFSET],
    ...
    {
        extendedHeaderLength = BufferTools.unpackSynchsafeInteger(bytes[offset], bytes[offset + 1],
                                                                  bytes[offset + 2],
                                                                  bytes[offset + 3]) + 4;
    ...
    {
        if (!FOOTER_TAG.equals(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes, offset,
                                                                                    FOOTER_TAG.
           length())))
        {
    ...
    private int packExtendedHeader(byte[] bytes, int offset)
    {
        ...
        BufferTools.copyIntoByteBuffer(extendedHeaderData, 0, extendedHeaderData.length, bytes,
                                       offset + 4);
    ...
    private int packSpecifiedFrames(byte[] bytes, int offset, String onlyId, String notId) throws
       NotSupportedException
    {
        ...
        while (setIterator.hasNext())
        {
               ...
               equals(frameSet.getId())))
            {
                ...
                {
                    ID3v2Frame frame = frameIterator.next();
                    if (frame.getDataLength() > 0)
                    {
                        byte[] frameData = frame.toBytes();
                        BufferTools.
                           copyIntoByteBuffer(frameData, 0, frameData.length, bytes, offset);
    ...
    {
        return Collections.unmodifiableMap(frameSets);
    }
    ...
    public void setUrl(String url)
    {
        ...
        {
            invalidateDataLength();
            ID3v2UrlFrameData frameData = new ID3v2UrlFrameData(useFrameUnsynchronisation(), null,
                                                                url);
    ...
    private ID3v2CommentFrameData extractCommentFrameData(String id, boolean itunes)
    {
        ...
        if (frameSet != null)
        {
            ...
            while (iterator.hasNext())
            {
                ...
                try
                {
                    frameData = new ID3v2CommentFrameData(useFrameUnsynchronisation(), frame.
                       getData());
                    if (itunes && ITUNES_COMMENT_DESCRIPTION.equals(frameData.getDescription().
                       toString()))
                    {
                        return frameData;
                    }
    ...
    private ID3v2PictureFrameData createPictureFrameData(String id)
    {
        ...
        if (frameSet != null)
        {
            ...
            try
            {
                if (obseleteFormat)
                {
                    frameData = new ID3v2ObseletePictureFrameData(useFrameUnsynchronisation(),
                                                                  frame.getData());
                }
                else
                {
                    frameData = new ID3v2PictureFrameData(useFrameUnsynchronisation(), frame.
                       getData());
                }
                return frameData;
            }
    ...
    {
        int hash = 7;
        hash = 71 * hash + (this.unsynchronisation ? 1 : 0);
        hash = 71 * hash + (this.extendedHeader ? 1 : 0);
        hash = 71 * hash + (this.experimental ? 1 : 0);
        hash = 71 * hash + (this.footer ? 1 : 0);
        hash = 71 * hash + (this.compression ? 1 : 0);
        hash = 71 * hash + Objects.hashCode(this.version);
        hash = 71 * hash + this.dataLength;
        hash = 71 * hash + this.extendedHeaderLength;
        hash = 71 * hash + Objects.hashCode(this.frameSets);
        return hash;
    }
}
