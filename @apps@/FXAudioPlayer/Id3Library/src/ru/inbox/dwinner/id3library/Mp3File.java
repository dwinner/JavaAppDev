...
public class Mp3File extends FileWrapper
{
   ...
   {
      try (RandomAccessFile _file = new RandomAccessFile(getFilename(), "r"))
      {
         initId3v1Tag(_file);
         scanFile(_file);
         if (startOffset < 0)
      ...
      try
      {
         ...
         if (bytesRead == AbstractID3v2Tag.HEADER_LENGTH)
         {
            try
            {
               ...
               return AbstractID3v2Tag.HEADER_LENGTH + BufferTools.
                 unpackSynchsafeInteger(bytes[AbstractID3v2Tag.DATA_LENGTH_OFFSET],
   ...
   private int scanBlock(byte[] bytes, int bytesRead, int absoluteOffset, int offset) throws
     InvalidDataException
   {
      while (offset < bytesRead - MINIMUM_BUFFER_LENGTH)
      {
                                                           ...
                                                           InvalidDataException
   {
      ...
      else
      {
         ...
         if (hasXingFrame())
         {
            _bufferLength = xingOffset;
         }
         else
         {
            _bufferLength = startOffset;
         }
         byte[] bytes = new byte[_bufferLength];
         file.seek(0);
         int bytesRead = file.read(bytes, 0, _bufferLength);
         if (bytesRead < _bufferLength)
         {
            throw new IOException("Not enough bytes read");
         }
         try
   ...
   {
      int _bufferLength = (int) (getLength() - (endOffset + 1));
      if (hasId3v1Tag())
      {
         _bufferLength -= ID3v1Tag.TAG_LENGTH;
      }
      if (_bufferLength <= 0)
      {
         customTag = null;
      ...
      {
         customTag = new byte[_bufferLength];
         file.seek(endOffset + 1);
         int bytesRead = file.read(customTag, 0, _bufferLength);
         if (bytesRead < _bufferLength)
         {
            throw new IOException("Not enough bytes read");
         }
   ...
   {
      return Collections.unmodifiableMap(bitrates);
   }
   ...
   {
      if (getFilename().compareToIgnoreCase(newFilename) == 0)
      {
   ...
   private void saveMpegFrames(RandomAccessFile saveFile) throws IOException
   {
      ...
      RandomAccessFile _file = new RandomAccessFile(getFilename(), "r");
      byte[] bytes = new byte[bufferLength];
      try
      {
         _file.seek(filePos);
         while (true)
         {
            int bytesRead = _file.read(bytes, 0, bufferLength);
