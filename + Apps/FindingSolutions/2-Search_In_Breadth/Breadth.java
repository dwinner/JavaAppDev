...
public class Breadth {
	...
	private FlightInfo find(String from) {
		for (int i=0; i < numFlights; i++) {
			if (flights[i].getFrom().equals(from) && !flights[i].isSkip()) {
				FlightInfo f = new FlightInfo(flights[i].getFrom(), flights[i].getTo(), flights[i].getDistance());
				flights[i].setSkip(true);
				return f;
			}
		}
	...
	// Определить, есть ли путь между from и to, используя поиск в ширину.
	private void isflight(String from, String to) {
		int dist, dist2;
		FlightInfo f;

		Stack resetStck = new Stack();	// Этот стек необходим при поиске в ширину.

		dist = match(from, to);	// Просмотр наличия пункта назначения.
		if (dist != 0) {
			btStack.push(new FlightInfo(from, to, dist));
			return;
		}

		// Проверка всех маршрутов из заданного узла.
		while ((f = find(from)) != null) {
			resetStck.push(f);
			if ((dist = match(f.getTo(), to)) != 0) {
				resetStck.push(f.getTo());
				btStack.push(new FlightInfo(from, f.getTo(), f.getDistance()));
				btStack.push(new FlightInfo(f.getTo(), to, dist));
				return;
			}
		}

		// Переустановка полей skip.
	...
	// Перестановка полей skip указанного маршрута.
	private void resetSkip(FlightInfo f) {
		for (int i=0; i< numFlights; i++)
