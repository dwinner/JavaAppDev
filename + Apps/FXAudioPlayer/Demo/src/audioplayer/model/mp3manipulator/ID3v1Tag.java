...
public class ID3v1Tag implements ID3v1
{
    ...
    private void unpackTag(byte[] bytes) throws NoSuchTagException
    {
        ...
        {
            comment = BufferTools.trimStringRight(BufferTools.
               byteBufferToStringIgnoringEncodingIssues(bytes, COMMENT_OFFSET, COMMENT_LENGTH_V1_0));
            track = null;
        }
        else
        {
            comment = BufferTools.trimStringRight(BufferTools.
               byteBufferToStringIgnoringEncodingIssues(bytes, COMMENT_OFFSET, COMMENT_LENGTH_V1_1));
    ...
    private void sanityCheckTag(byte[] bytes) throws NoSuchTagException
    {
        ...
        if (!TAG.
           equals(BufferTools.byteBufferToStringIgnoringEncodingIssues(bytes, 0, TAG.length())))
        {
    ...
    private void packField(byte[] bytes, String value, int maxLength, int offset)
    {
        if (value != null)
        {
            try
            {
                BufferTools.stringIntoByteBuffer(value, 0, Math.min(value.length(), maxLength),
                                                 bytes, offset);
            }
            catch (UnsupportedEncodingException e)
            {
