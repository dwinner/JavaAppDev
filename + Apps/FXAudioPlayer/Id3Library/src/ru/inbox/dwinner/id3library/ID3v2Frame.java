...
public class ID3v2Frame
{
   ...
   private void unpackFlags(byte[] buffer, int offset)
   {
      ...
      unsynchronisation = BufferTools.checkBit(buffer[offset + FLAGS2_OFFSET],
                                               UNSYNCHRONISATION_BIT);
      dataLengthIndicator = BufferTools.checkBit(buffer[offset + FLAGS2_OFFSET],
   ...
   protected void sanityCheckUnpackedHeader() throws InvalidDataException
   {
      ...
      {
         if (!((id.charAt(i) >= 'A' && id.charAt(i) <= 'Z') || (id.charAt(i) >= '0' && id.charAt(
           i) <= '9')))
         {
   ...
   public int hashCode()
   {
      ...
      hash = 79 * hash + (this.preserveTag ? 1 : 0);
      hash = 79 * hash + (this.preserveFile ? 1 : 0);
      hash = 79 * hash + (this.readOnly ? 1 : 0);
      hash = 79 * hash + (this.group ? 1 : 0);
      hash = 79 * hash + (this.compression ? 1 : 0);
      hash = 79 * hash + (this.encryption ? 1 : 0);
      hash = 79 * hash + (this.unsynchronisation ? 1 : 0);
      hash = 79 * hash + (this.dataLengthIndicator ? 1 : 0);
      return hash;
   }
}
