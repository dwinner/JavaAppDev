...
public class Block
{
    //    private static BufferedImage sprites = Utils.getBufferedImage (
    //            new ImageIcon ( ImageUploader.class.getResource ( "icons/about/sprites.png" ) ) );

    //    private static final int SPRITES_SIDE = 50;
    ...
    //    private ImageIcon getRandomSprite ()
    //    {
    //        return new ImageIcon ( sprites.getSubimage ( random.nextInt ( SPRITES_SIDE ) * 16,
    //                random.nextInt ( SPRITES_SIDE ) * 16, 16, 16 ) );
    //    }
    ...
    public void paintBlock ( Graphics2D g2d, Rectangle rect, int rotation )
    {
        ...
        // Поправка при вращении:
        // 3 = x+1
        // 2 = x+1 y+1
        // 1 = y+1
        RoundRectangle2D rr = new RoundRectangle2D.Double (
                -rect.width / 2 + ( rotation == 2 || rotation == 3 ? -1 : 0 ),
                -rect.height / 2 + ( rotation == 1 || rotation == 2 ? -1 : 0 ), rect.width,
                rect.height, 4, 4 );
        Rectangle bounds = rr.getBounds ();

        // Фон
        g2d.setPaint (
                new GradientPaint ( 0, bounds.y, bgTop, 0, bounds.y + bounds.height, bgBottom ) );
        g2d.fill ( rr );
        ...
        if ( text != null )
        {
            ...
            g2d.drawString ( text,
                    bounds.x + bounds.width / 2 - g2d.getFontMetrics ().stringWidth ( text ) / 2 +
                            1,
        ...
        else if ( icon != null )
        {
            g2d.drawImage ( icon.getImage (),
                    bounds.x + bounds.width / 2 - icon.getIconWidth () / 2 + 1,
