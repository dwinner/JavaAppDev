...
public class WebImageGallery extends JComponent
{
    ...
    public WebImageGallery ()
    {
        ...
        setFocusable ( true );
        setFont ( new JLabel ().getFont ().deriveFont ( Font.BOLD ).deriveFont ( 12f ) );
            ...
            view = new WebScrollPane ( WebImageGallery.this, withBorder )
            {
                public Dimension getPreferredSize ()
                {
                    ...
                    return new Dimension ( spacing * ( columns + 1 ) + columns * maxWidth,
                            WebImageGallery.this.getPreferredSize ().height +
                                    WebScrollBarUI.LENGTH );
                }
            };
    ...
    public void setSelectedIndex ( int selectedIndex )
    {
        ...
        if ( scrollOnSelection )
        {
            ...
            SwingUtils.scrollSmoothly ( getView (),
                    rect.x + rect.width / 2 - WebImageGallery.this.getVisibleRect ().width / 2,
                    rect.y );
        }
        moveReflection ();
    }
    ...
    private void moveReflection ()
    {
        ...
        progress = 0f;
        reflectionMover = new Timer ( 1000 / 48, new ActionListener ()
        {
    ...
    public void addImage ( int index, ImageIcon image )
    {
        try
        {
            ...
            rg.drawImage ( previewIcon.getImage (), 0, 0, null );
            rg.setComposite ( AlphaComposite.getInstance ( AlphaComposite.DST_IN ) );
            rg.setPaint (
                    new GradientPaint ( 0, rheight * ( 1f - fadeHeight ), new Color ( 0, 0, 0, 0 ),
                            0, rheight, new Color ( 0, 0, 0, opacity ) ) );
            rg.fillRect ( 0, 0, rwidth, rheight );
            rg.dispose ();

            images.add ( index, previewIcon );
            descriptions
                    .add ( index, image.getIconWidth () + " x " + image.getIconHeight () + " px" );
    ...
    protected void paintComponent ( Graphics g )
    {
        ...
        for ( int i = 0; i < images.size (); i++ )
        {
            ...
            float add = selectedIndex == i ? progress * 0.4f :
            ...
            Area gp = new Area ( new RoundRectangle2D.Double ( x - imageWidth / 2 - borderWidth,
                    y - imageHeight / 2 - borderWidth, imageWidth + borderWidth * 2,
                    imageHeight + borderWidth * 2, borderWidth * 2, borderWidth * 2 ) );
            gp.subtract ( new Area (
                    new Rectangle ( x - imageWidth / 2, y - imageHeight / 2, imageWidth,
                            imageHeight ) ) );
            g2d.fill ( gp );

            g2d.setComposite ( oldComposite );

            // Информационный текст над отражением
            ...
            //                    RenderingHints.VALUE_ANTIALIAS_ON );
            //
            //            rg.drawImage ( bi, borderWidth, borderWidth, null );
            //            rg.setComposite ( AlphaComposite.getInstance ( AlphaComposite.DST_IN ) );
            //            rg.setPaint ( new GradientPaint ( 0, rheight * fadeHeight, new Color ( 0, 0, 0, 0 ), 0,
            //                    rheight, new Color ( 0, 0, 0, opacity ) ) );
            //            rg.fillRect ( 0, 0, rwidth, rheight );
            //
            //            rg.setComposite ( AlphaComposite.getInstance ( AlphaComposite.SRC_OVER ) );
            //            rg.setPaint ( new GradientPaint ( 0, rheight * (1f-fadeHeight),
            //                    selectedIndex == i ? selectedTransparent : transparent, 0, rheight,
            //                    selectedIndex == i ? selectedLight : light ) );
            //            gp = new Area ( new RoundRectangle2D.Double ( 0, 0, rwidth, rheight, borderWidth * 2,
            //                    borderWidth * 2 ) );
            //            gp.subtract ( new Area (
            //                    new Rectangle ( borderWidth, borderWidth, rwidth - borderWidth * 2,
            ...
            {
                g2d.drawImage ( reflections.get ( i ), x - imageWidth / 2,
                        y2 + imageHeight / 2 + addition, imageWidth, -imageHeight, null );
            }

            gp = new Area (
                    new RoundRectangle2D.Double ( x - rwidth / 2, y2 - rheight / 2 + addition,
                            rwidth, rheight, borderWidth * 2, borderWidth * 2 ) );
            gp.subtract ( new Area ( new Rectangle ( x - rwidth / 2 + borderWidth,
                    y2 - rheight / 2 + addition + borderWidth, rwidth - borderWidth * 2,
                    rheight - borderWidth * 2 ) ) );
            g2d.setPaint ( new GradientPaint ( 0, y2 - imageHeight / 2 + addition,
                    selectedIndex == i ? selectedLight : light, 0,
                    y2 - imageHeight / 2 + addition + imageHeight * fadeHeight,
                    selectedIndex == i ? selectedTransparent : transparent ) );
            g2d.fill ( gp );
        }
    }
    ...
    public Dimension getPreferredSize ()
    {
        return new Dimension ( spacing * ( images.size () + 1 ) + maxWidth * images.size (),
                spacing * 3 + maxHeight * 2 );
    }
}
