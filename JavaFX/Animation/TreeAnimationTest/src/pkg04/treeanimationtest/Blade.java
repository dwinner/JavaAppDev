...
{
    public final Color SPRING_COLOR = Color.
       color(random() * 0.5, random() * 0.5 + 0.5, 0.).darker();
    public final Color AUTUMN_COLOR = Color.color(random() * 0.4 + 0.3,
                                                  random() * 0.1 + 0.4, random() * 0.2);
    ...
    public Blade()
    {
        ...
        curve1.xProperty().bind(new DoubleBinding()
        {
            ...
            protected double computeValue()
            {
                ...
                return (h / 4) +
                   ((cos(phase.get() + (x + 400.) * PI / 1600 + rand) + 1) / 2.) *
                   (-3. / 4) * h;
            }
        });
    }
