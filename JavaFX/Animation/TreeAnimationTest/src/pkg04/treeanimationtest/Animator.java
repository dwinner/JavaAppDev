...
public class Animator implements Runnable
{
    ...
    public void run()
    {
        ...
        for (int i = 0; i < tree.generations.size(); i++)
        {
            ...
            branchGrowingAnimation.getChildren().add(
               animateBranchGrowing(branchGeneration,
                                    i,
                                    BRANCH_GROWING_DURATION)); //create animation for current crown
            treeWindAnimation.getChildren().add(
               animateTreeWind(branchGeneration,
                               i,
                               WIND_CYCLE_DURATION));
        }

        // Main animation: grass bending, tree bending, tree growing, seasons changing
        final Transition all = new ParallelTransition(
           new GrassWindAnimation(grass),
           treeWindAnimation,
           new SequentialTransition(branchGrowingAnimation,
                                           ...
                                           Duration duration)
    {
        ...
        for (final Branch branch : branchGeneration)
        {
            Timeline branchGrowingAnimation = new Timeline(
               //line is growing by changinh endY from 0 to brunch.length
            ...
            sameDepthBranchAnimation.getChildren().add(
               ...
               //trick to show lines
               PauseTransitionBuilder.create().duration(Duration.ONE).
    ...
    private Animation animateTreeWind(List<Branch> branchGeneration,
                                      int depth,
                                      Duration duration)
    {
        ...
        for (final Branch brunch : branchGeneration)
        {
            ...
            wind.getChildren().add(TimelineBuilder.create().keyFrames(new KeyFrame(
               duration,
               new KeyValue(rotation.angleProperty(), depth * 2))).autoReverse(true).
               cycleCount(
               Animation.INDEFINITE).build());
        }
        return wind;
    }
    ...
    private Transition seasonsAnimation(final Tree tree, final List<Blade> grass)
    {
        ...
        return SequentialTransitionBuilder.create().children(spring, flowers, autumn).
        ...
        for (int i = 0; i < flowers.size(); i++)
        {
            ...
            for (Ellipse pental : flower.getPetals())
            {
                flowersAppearAndFallDown.getChildren().add(new SequentialTransition(
                   //flowers appearing
                   FadeTransitionBuilder.create().delay(FLOWER_APPEARING_DURATION.
                   divide(3).multiply(i + 1)).duration(FLOWER_APPEARING_DURATION).node(
                   pental).toValue(1).build(),
                   //fall down
    ...
    private Transition animateAutumn(List<Leaf> leafage, List<Blade> grass)
    {
        ...
        for (final Leaf leaf : leafage)
        {
            ...
            final FillTransition toYellow = FillTransitionBuilder.create().shape(leaf).
               toValue(leaf.AUTUMN_COLOR).duration(LEAF_BECOME_YELLOW_DURATION).build();
            yellowLeafage.getChildren().add(toYellow);

            //fall down
            Animation fakeLeafageDown = fakeFallDownEllipseAnimation((Ellipse) leaf,
                                                                     leaf.AUTUMN_COLOR,
                                                                     new HideMethod()
            {
                @Override
            ...
            dissappearLeafage.getChildren().add(new SequentialTransition(
               fakeLeafageDown,
               FillTransitionBuilder.create().shape(leaf).toValue((Color) leaf.getFill()).
               duration(Duration.ONE).build()));
        }
        ...
        {
            //become yellow            
            final FillTransition toYellow = FillTransitionBuilder.create().shape(blade).
               toValue(blade.AUTUMN_COLOR).delay(Duration.seconds(1 * random())).duration(
               GRASS_BECOME_YELLOW_DURATION).build();
            grassBecomeYellowAnimation.getChildren().add(toYellow);
        }

        autumn.getChildren().addAll(grassBecomeYellowAnimation, new SequentialTransition(
    ...
    private Animation fakeFallDownEllipseAnimation(final Ellipse sourceEllipse,
                                                   Color fakeColor,
                                                   final HideMethod hideMethod)
    {
        final Ellipse fake = copyEllipse(sourceEllipse, fakeColor);
        addChildToParent(treeGenerator.content, fake);

        PauseTransition replaceFakeWithSource = PauseTransitionBuilder.create().
           duration(Duration.ONE).delay(Duration.minutes(0.9 * random() + 0.1)).
           onFinished(new EventHandler<ActionEvent>()
        {
            @Override
            public void handle(ActionEvent t)
            {
                ...
                final Point2D position = treeGenerator.content.sceneToLocal(
                   sourceEllipse.localToScene(0, 0));
                ...
                {                    
                    ...
                    {
                        return 50 * sin((fake.translateYProperty().doubleValue() - position.
                           getY()) / 20);
                    }
                };
                ...
                fake.rotateProperty().bind(
                   fake.translateYProperty().multiply(2).add(random() * 180));
        ...
        return new SequentialTransition(replaceFakeWithSource,
           ...
           duration(Duration.seconds(30)).toY(random() * 30 + 1).node(fake).build(),
                                        //disappear
                                        FadeTransitionBuilder.create().toValue(0).
           delay(Duration.seconds(5)).duration(Duration.seconds(2)).node(fake).build());
    }
